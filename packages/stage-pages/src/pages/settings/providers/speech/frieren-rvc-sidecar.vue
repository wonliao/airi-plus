...
import type { FrierenSpeechSidecarStatusResult } from '@proj-airi/stage-shared'
...
const sidecarActionLoading = ref<'import' | 'install' | 'restart' | null>(null)
...
watch([bridgeDir, envFile, kokoroProjectDir, rvcModelPath, rvcIndexPath, baseUrl], () => {
  debouncedRefreshSidecarStatus()
})

watch([isValid, bridgeDir, envFile, kokoroProjectDir, rvcModelPath, rvcIndexPath, baseUrl], async ([valid]) => {
  if (!valid) {
    return
  }
