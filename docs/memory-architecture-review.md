...
- remote Mem0 設定與 recall / capture policy 還需要持續收斂
...
目前 short-term memory 已收斂為 remote Mem0 client integration，capture 仍保留：
...
- validation
- remote API proxy / connectivity probe
- search
- capture
- clear / list
...
### Remote Mem0-only mode simplifies the product model

把 short-term memory 收斂成 remote Mem0-only，讓設定、驗證與執行路徑的心智一致。

它的好處是：

- AIRI 不再承擔本地 memory backend 的 provider 與儲存責任
- UI 欄位能對應到真正生效的 backend 行為
- Docker 端可以獨立演進 provider、embedding 與儲存配置
...
### Risk 4: Remote Mem0 contract drift

UI、store 與 Electron service 現在都已經改成 remote Mem0 client 模式，但後續仍要注意 AIRI 與 Docker Mem0 API 契約不要漂移。
...
- Docker 端 API shape 變動時，AIRI 的 client helper 需要同步更新
- 若 remote Mem0 的 scope 或 auth policy 調整，UI 驗證文案可能失真
- debug panel 顯示的是 AIRI 端觀察到的 request / result，不是 Docker 端完整內部狀態
...
當時的 local runtime 與 external mem0 config 產品心智混在一起。

#### Refactor Direction

- 收斂為單一 remote Mem0 mode
- 只展示真正會生效的 remote connection 欄位
