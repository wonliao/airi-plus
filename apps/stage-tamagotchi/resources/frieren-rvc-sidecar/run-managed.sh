...
export FORCE_RVC_DEVICE="${FORCE_RVC_DEVICE:-}"
...
if [[ "$KOKORO_SYNTH_MODE" == "embedded" ]]; then
  ...
  export PRELOAD_MODEL_ON_STARTUP="${PRELOAD_MODEL_ON_STARTUP:-false}"
  export ENABLE_STARTUP_WARMUP="${ENABLE_STARTUP_WARMUP:-false}"
